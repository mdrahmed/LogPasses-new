...
bool CPSTracker::runOnModule(Module &M) {
	...
	for (auto &F:M){
		...
		if(F.getName().contains("publishVGR_Do") || F.getName().contains("main")){
			outs()<<"publishVGR_do found\n";
