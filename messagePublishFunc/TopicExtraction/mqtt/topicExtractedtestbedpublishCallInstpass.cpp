				...
				for(Instruction &I: BB){
					...
					if(auto *invokeInst = dyn_cast<InvokeInst>(&I)){
						...
						if (Function *calledFunction = dyn_cast<Function>(calledValue)) {
							StringRef functionName = calledFunction->getName();
							outs() << "Invoke Function Name: " << functionName << "\n";
							...
							if(functionName.contains("make_message")){
								...
								//outs()<<"argOperand: "<<invokeInst->getArgOperand(0)<<"\n";
								//BasicBlock* successBlock = invokeInst->getNormalDest();
								//IRBuilder<> builderInvoke(successBlock->getFirstNonPHI());


								//for(Instruction &In: *successBlock){
								//	//_ZNKSt7__cxx1112basic_stringIcSt11char_traitsIcESaIcEE5c_strEv
								//	if(auto *callInst = dyn_cast<CallInst>(&In)) {
                                        			//		Function *calledFunction = callInst->getCalledFunction();
								//		//if(calledFunction->getName() == "_ZNSt7__cxx1112basic_stringIcSt11char_traitsIcESaIcEED1Ev"){
								//		if(calledFunction->getName().contains("mqtt10buffer_ref")){
								//			outs()<<"Found expected function: "<<calledFunction->getName()<<"\n";
								//			//if (callInst == successBlock->getTerminator()) {
                                                              	//			//    builderInvoke.SetInsertPoint(successBlock, ++successBlock->end());
                                                              	//			//} else {
                                                              	//			//    builderInvoke.SetInsertPoint(successBlock, ++I.getIterator());
                                                              	//			//}
								//			builderInvoke.SetInsertPoint(callInst->getParent(), ++callInst->getIterator());
								//			llvm::Function* c_str = F.getParent()->getFunction("_ZNKSt7__cxx1112basic_stringIcSt11char_traitsIcESaIcEE5c_strEv");
								//			//_ZNSt7__cxx1112basic_stringIcSt11char_traitsIcESaIcEED1Ev
								//			//llvm::Function* c_str = F.getParent()->getFunction("_ZStlsISt11char_traitsIcEERSt13basic_ostreamIcT_ES5_PKc");
                                                             	//			outs()<<"c_str(): "<< *c_str<<"\n";
                                                             	//			Value *topicStr = builderInvoke.CreateCall(c_str, callInst);

								//			if(c_str){
								//				std::string formatValue("make_message value: ");
                                                                //      				formatValue += " %s\n";
                                                                //      				Value *value = builder.CreateGlobalStringPtr(formatValue, "value");
                                                                //      				std::vector<Value *> argsValue({value});
                                                                //      				//argsValue.push_back(value);
                                                                //      				argsValue.push_back(topicStr);
                                                                //      				builderInvoke.CreateCall(printfFunc, argsValue, "value");
								//			}
								//		}
								//	}
								//}
								for (unsigned int i = 0; i < numOperands; ++i) {
									Value *operand = invokeInst->getOperand(i);
									//outs()<<"operand "<<i<<": "<<*invokeInst->getOperand(i)<<"\n";
									// Create an IRBuilder for the successor block
									//BasicBlock* successBlock = invokeInst->getNormalDest();
									//IRBuilder<> builderInvoke(successBlock->getFirstNonPHI());
									...
									//	if(auto *callInst = dyn_cast<CallInst>(&In)) {
                                        				//		Function *calledFunction = callInst->getCalledFunction();
									//		//if(calledFunction->getName() == "_ZNSt7__cxx1112basic_stringIcSt11char_traitsIcESaIcEED1Ev"){
									//		if(calledFunction->getName().contains("mqtt10buffer_ref") || calledFunction->getName() == "_ZNSt7__cxx1112basic_stringIcSt11char_traitsIcESaIcEED1Ev"){
									//			outs()<<"Found expected function: "<<calledFunction->getName()<<"\n";
									//			//if (callInst == successBlock->getTerminator()) {
                                                              		//			//    builderInvoke.SetInsertPoint(successBlock, ++successBlock->end());
                                                              		//			//} else {
									...
									// This part is inserting values but it will not print the message, so, i need to find a more solid way to print the values
									if (auto *allocaInst = dyn_cast<AllocaInst>(operand)) {
										//outs() << "Operand " << i << " is an AllocaInst: " << *allocaInst << "\n";
										IRBuilder<> builderInvoke(invokeInst);
										// Get the successor block for successful execution (label %416)
										BasicBlock* successBlock = invokeInst->getNormalDest();
										// Insert code at the successor block
										builderInvoke.SetInsertPoint(successBlock->getFirstNonPHI());

										Value *loadedValue = builderInvoke.CreateLoad(allocaInst->getType()->getPointerElementType(), allocaInst);
										Type *allocaType = allocaInst->getAllocatedType();
										//outs()<<"allocaType: "<<*allocaType<<"\n";
										outs()<<"loadedValue: "<<*loadedValue<<"\n";
										// Check if the type is a pointer type
										if (PointerType *ptrType = dyn_cast<PointerType>(allocaType)) {
										    // Check if the element type of the pointer is an integer type
										    if (ptrType->getElementType()->isIntegerTy()) {
										        //outs() << "Operand " << i << " is a string\n";
										
										        // Proceed with handling the string value
										    }
										}
										//if (invokeInst == BB.getTerminator()) {
                                                              			//    builder.SetInsertPoint(&BB, ++BB.end());
										...
										//builderInvoke.SetInsertPoint(invokeInst->getParent(), ++invokeInst->getIterator());
										llvm::Function* c_str = F.getParent()->getFunction("_ZNKSt7__cxx1112basic_stringIcSt11char_traitsIcESaIcEE5c_strEv");
                                                             			//outs()<<"c_str(): "<< *c_str<<"\n";
                                                             			Value *topicStr = builderInvoke.CreateCall(c_str, loadedValue);

										std::string formatValue("make_message2 value: ");
										formatValue += " %s\n";
										Value *value = builder.CreateGlobalStringPtr(formatValue, "value");
										std::vector<Value *> argsValue({value});
										//argsValue.push_back(value);
						...
						//outs()<<"numOperands: "<<numOperands<<"\n";
						//outs()<<calledFunction<<"\n";
						//if(calledFunction->getName().contains("make_message")){
						//	outs()<<"found make_message: "<<*invokeInst<<"\n";
						//	//outs()<<"argOperand: "<<invokeInst->getArgOperand(0)<<"\n";
						//	//for (unsigned int i = 0; i < numOperands; ++i) {
						//	//	outs()<<"operand "<<i<<": "<<invokeInst->getArgOperand(i);
						//	//}
						//}
					}
					...
					//	if(calledFunction){
					//		if(calledFunction->getName().contains("make_message")){
					//			outs()<<"found make_message: "<<*callInst<<"\n";
					//		}
