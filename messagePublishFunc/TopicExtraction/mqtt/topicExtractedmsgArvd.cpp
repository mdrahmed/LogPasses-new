							...
							if (containedType->getStructName().startswith("class.std::shared_ptr")) {
							   { // This part checks if v is an actual object, not necessarily required here, but yet given to ensure the type of v
								...
								if(mqttMsgFn){
									...
									//Type *charPtrType = Type::getInt8PtrTy(context);
									//std::vector<Type *> cstrArgsTypes({charPtrType});
									//FunctionType *cstrType = FunctionType::get(charPtrType, cstrArgsTypes, false);
									//Function *cstrFunc = cast<Function>(M.getOrInsertFunction("c_str", cstrType).getCallee());
									//outs()<<"cstrFunc: "<< *cstrFunc << "\n";
									//std::string hello = "hello";
									//outs()<<hello.c_str()<<"\n";

									//std::string valueStr = "Topic is instrumented, topic: ";
