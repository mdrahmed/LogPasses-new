...
std::set<std::string> callInstFuncNames;

bool isNamePresent(std::string name) {
    auto it = std::find(callInstFuncNames.begin(), callInstFuncNames.end(), name);
    return it != callInstFuncNames.end();
}
	...
	for (auto &F:M){	
		...
		for (BasicBlock &BB : F) {
			...
			for(Instruction &I: BB){
				...
				if(isFunc && isNamePresent(F.getName().str())){
					...
					isFunc = false;
					//outs()<<"Check if print is inserted: "<<F<<"\n";
				}
				//// only logging CallInst functions
				//if(auto *callInst = dyn_cast<CallInst>(&I)){
				//	Function *calledFunction = callInst->getCalledFunction();
				//	if(calledFunction){
				//		std::string functionName = calledFunction->getName().str();
				//		callInstFuncNames.insert(functionName);
				//		//outs()<<"call inst funciton names: "<< functionName <<"\n";
				//		//outs()<<"calledFunction: "<<*calledFunction<<"\n";
				//	}
				//}
				...
				if(ifCallInst){
					if(auto *callInst = dyn_cast<CallInst>(&I)) {
						...
						if (calledFunction && calledFunction->getName() != "printf") {
							...
							std::string functionName = calledFunction->getName().str();
							callInstFuncNames.insert(functionName);
							outs()<<"Called function: "<<functionName<<"\n";
								...
								std::string intV("callInst values: ");
