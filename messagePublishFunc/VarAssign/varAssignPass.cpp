			...
			for(Instruction &I: BB){
				if (auto* inst = dyn_cast<StoreInst>(&I)) {
					Value *storedValue = inst->getValueOperand();
  					Value *pointerOperand = inst->getPointerOperand();
  					if (auto *allocaInst = dyn_cast<AllocaInst>(pointerOperand)) {
  					  // We have an AllocaInst, so we can print the variable name and the stored value
  					  outs() << "Storing value " << *storedValue << " in variable " << allocaInst->getName() << "\n";
  					}
					outs()<<"Store Inst: "<<*inst<<"\n";
