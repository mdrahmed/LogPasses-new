//functionValuesAsString.cpp - Trying to get the values as string if it's in string
		...
		if(F.getName().contains("message_arrived")) {
			...
			if(!F.isDeclaration()){
				...
				
				{
					...
					std::vector<Type*> arg_types = F.getFunctionType()->params();
					for (size_t i = 0; i < arg_values.size(); ++i) {
						outs()<<*arg_values[i]->getType()<<"\n";
						Type *type = arg_values[i]->getType();
						if (type->getTypeID() == Type::PointerTyID && type->getPointerElementType()->isStructTy()) {
    							StructType *structType = cast<StructType>(type->getPointerElementType());
    							if (structType->getName().startswith("class.std::shared_ptr")) {
								outs()<<"Found this type: class.std::shared_ptr\n";
							}
						}
	    
						//if (auto* ptrType = dyn_cast<PointerType>(arg_values[i]->getType())) {
						//    if (auto* elemType = dyn_cast<IntegerType>(ptrType->getElementType())) {
						//        if (elemType->getBitWidth() == 8) {
						//            // Pointer to char, i.e. string
						//            outs() << "Value is a string\n";
						//        }
						//    }
						//}
					        Type *argType = arg_types[i];
					        if (argType->isIntegerTy() && argType->getIntegerBitWidth() == 32) {
					            format += "%d\n";
					            //argsV.push_back(builder.CreateGlobalStringPtr(arg_values[i]->getName(), ""));
					            argsV.push_back(arg_values[i]);
					        } 
					        else if (argType->isPointerTy() && argType->getPointerElementType()->isIntegerTy(8)) {
					                outs()<<"In string part\n";
					                format += "%s\n";
					           	    //argsV.push_back(builder.CreateGlobalStringPtr(arg_values[i]->getName(), ""));
					           	    argsV.push_back(arg_values[i]);
					           	    std::vector<Value*> printfArgs({builder.CreateGlobalStringPtr("%s\n", ""), arg_values[i]});
